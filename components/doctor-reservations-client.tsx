...
import { Plus, Pencil, Trash2, ClipboardPlus, Ban } from "lucide-react";
...
interface DoctorReservationsClientProps {
  reservations: Reservation[];
  patients: { id: string; fullName: string; phoneNumber?: string | null }[];
  dict: import("@/lib/i18n/types").Dictionary;
}

export function DoctorReservationsClient({ reservations, patients, dict }: DoctorReservationsClientProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-foreground">{dict.navigation.reservations}</h1>
          <p className="mt-1 text-sm text-muted-foreground">{dict.reservations.subtitle}</p>
        </div>
        <Button
          onClick={() => { setEditReservation(undefined); setFormOpen(true); }}
          className="bg-primary hover:bg-primary/90 text-primary-foreground shadow-sm shadow-primary/20"
        >
          <Plus className="me-2 h-4 w-4" /> {dict.reservations.create}
        </Button>
      </div>
          ...
          <div className="flex gap-1 justify-end">
            {row.status === "scheduled" && (
              <Button
                ...
                onClick={() => {
                  router.push(`/doctor/consultations/new?patientId=${row.patientId}&reservationId=${row.id}`);
                }}
              >
